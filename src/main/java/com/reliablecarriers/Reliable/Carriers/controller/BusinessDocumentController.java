...
/**
 * Controller for business document uploads
 */
@RestController
...
public class BusinessDocumentController {
    ...
    public ResponseEntity<Map<String, Object>> uploadDocument(
        ...
        try {
            ...
            BusinessDocumentType documentType = BusinessDocumentType.valueOf(documentTypeStr);
            ...
            BusinessDocument document = documentService.uploadDocument(business, documentType, file, 
                isCertified, certifiedBy, certificationDate, expiresAt);
            
            // Update business status if documents submitted
    ...
    public ResponseEntity<Map<String, Object>> deleteDocument(
        ...
        try {
            User business = getAuthenticatedBusiness(authentication);
            boolean deleted = documentService.deleteDocument(documentId, business);
     ...
     * GET /api/business/documents/required
     */
    @GetMapping("/required")
    public ResponseEntity<Map<String, Object>> getRequiredDocuments() {
        List<Map<String, Object>> requiredDocs = Arrays.stream(BusinessDocumentType.values())
            .filter(BusinessDocumentType::isRequired)
            .map(docType -> {
        ...
        List<Map<String, Object>> optionalDocs = Arrays.stream(BusinessDocumentType.values())
    ...
    private User getAuthenticatedBusiness(Authentication authentication) {
        if (authentication == null || authentication.getName() == null) {
        ...
        if (user.getIsBusiness() == null || !user.getIsBusiness()) {
            throw new SecurityException("Only business accounts can access this endpoint");
        }
